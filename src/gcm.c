...
static int gcm_gen_table( gcm_context *ctx )
{
    ...
    memset( h, 0, 16 );
	// use block cipher to encrypt h	
	//
    //if( ( ret = cipher_update( &ctx->cipher_ctx, h, 16, h, &olen ) ) != 0 )
     //   return( ret );
	aes_context *tmp_ctx = &ctx->aes_ctx;
	aes_crypt_ecb(tmp_ctx, AES_ENCRYPT,h, h);
...
int gcm_init( gcm_context *ctx, const unsigned char *key,
              unsigned int keysize )
{
	...
	*/
	aes_context *tmp_ctx = &ctx->aes_ctx;
	aes_setkey_enc(tmp_ctx,key, keysize);
                ...
                size_t iv_len)
{
	...
	*/
	//ctx->y stores iv
	//is ctx->base_ectr= Ek(iv) ?
	aes_context *tmp_ctx=&ctx->aes_ctx;
	aes_crypt_ecb(tmp_ctx,AES_ENCRYPT,ctx->y,ctx->base_ectr);
	//
	//store gcm_mult(auth_data,h) to ctx->buf
	/*
    ctx->add_len = add_len;
    p = add;
    while( add_len > 0 )
...
/*
 *GHASH simulator
 *\param output: the output array of gcm_mult, Y1 to Yn
 \param ctx->buf: The output block of each gcm_mult, after calling this func, 
 	ghash = ctx-> buf
 * */
int gcm_update( gcm_context *ctx,
                size_t length,
                const unsigned char *input,//plaintext
                unsigned char *output ) //Y block
{
    int ret;
    ...
    while( length > 0 )
    {
		...
		// store Ek(counter i) to ectr , useless in gmac
		...
		*/
		//1. generate a ciphertext block, store into output by out_p
		//2. for encryption, xor the ciphertext block with the output of 
		//previous gcm_mult
        for( i = 0; i < use_len; i++ )
        {
			/*
            if( ctx->mode == GCM_DECRYPT )
                ctx->buf[i] ^= p[i];
            out_p[i] = ectr[i] ^ p[i]; //store the ciphertext, not necessary in gmac
			*/
			//ctx->buf stores the output of gcm_mult
            if( ctx->mode == GCM_ENCRYPT )
                //ctx->buf[i] ^= out_p[i];
				ctx->buf[i] ^= p[i];
        }

		//now, ctx->buf stores ciphertext block
        gcm_mult( ctx, ctx->buf, ctx->buf );
		//now, ctx->buf stores the output of gcm_mult
        ...
        length -= use_len;
        p += use_len; //plaintext
        out_p += use_len; //ciphertext
    }
	//the output of GHASH is stored in ctx->buf
	//output array stores Y1 to Yn
    return( 0 );
}
                ...
                size_t tag_len )
{
    ...
    if( orig_len  )
    {
		...
		//form the len(A) || len(C) , store it to work_buf
        //PUT_UINT32_BE( ( orig_add_len >> 32 ), work_buf, 0  );
        //PUT_UINT32_BE( ( orig_add_len       ), work_buf, 4  );
        PUT_UINT32_BE( ( orig_len     >> 32 ), work_buf, 0  );
        PUT_UINT32_BE( ( orig_len           ), work_buf, 4 );
		PUT_UINT32_BE( ( 0     >> 32 ), work_buf, 8  );
        PUT_UINT32_BE( ( 0           ), work_buf, 12 );

		//ctx->buf contains the output of ghash
        for( i = 0; i < 16; i++ )
            ctx->buf[i] ^= work_buf[i];

        gcm_mult( ctx, ctx->buf, ctx->buf );
		//ctx->buf now stores b_test block, store is to the file
		

		//xor Ek(iv) with output of gcm_mult
        for( i = 0; i < tag_len; i++ )
            tag[i] ^= ctx->buf[i];
    }
	//tag now stores c_test block, store it to file
                       ...
                       unsigned char *tag )
{
    ...
    if( ( ret = gcm_starts( ctx, mode, iv, iv_len) ) != 0 )
        return( ret );

    if( ( ret = gcm_update( ctx, length, input, output ) ) != 0 )
        return( ret );
	//store output as Y to file

    if( ( ret = gcm_finish( ctx, tag, tag_len ) ) != 0 )
        return( ret );
	//store ctx->buf as B, tag as tag to files
...
int gmac_selfTest()
{
	...
	int key_len = 128 + 64*0;
	gcm_init(&ctx, key[key_index[0]],key_len);
	ret = gcm_crypt_and_tag(&ctx,GCM_ENCRYPT, 
			pt_len[0],
			iv[iv_index[0]],iv_len[0],
			pt[pt_index[0]],buf,16,tag_buf);
	printf("%d\n",ret);
	return 0;
}
*/
